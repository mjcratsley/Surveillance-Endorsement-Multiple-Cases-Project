...
surveillancedt_long %>% drop_na(SOA)
surveillancedt_long %>% drop_na(Endorsement)
surveillancedt_long %>% drop_na(Case)

##for some reason I'm now once again getting an error with the ranova function: 
#Error in anova.merMod(surveillance.null, surveillance.model) : models were not all fitted to the same size of dataset
...
```{r Main Model}
surveillance.model <- lmer( Endorsement ~ SOA + (1 | ResponseId)+ (1 | Case), data=surveillancedt_long)
...
boxplot(Endorsement ~ Case,
        col=c("white","lightgray"),surveillancedt_long)
...
surveillance.null <- lmer(Endorsement ~ (1 | ResponseId) + (1 | Case), data=surveillancedt_long)
...
surveillance.model2 <- lmer(Endorsement ~ SOA + (SOA | ResponseId)+ (1 | Case), data=surveillancedt_long)
...
surveillance.model3 <- lmer(Endorsement ~ SOA + (1 | ResponseId)+ (SOA | Case), data=surveillancedt_long)
...
surveillance.model4 <- lmer(Endorsement ~ SOA + (SOA | ResponseId)+ (SOA | Case), data=surveillancedt_long)
